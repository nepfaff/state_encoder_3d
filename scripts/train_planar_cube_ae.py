...
OUT_PATH = f"outputs/planar_cube_ae_{time.strftime('%Y-%b-%d-%H-%M-%S')}/checkpoints"
...
def main():
    ...
    wandb.init(
        project="state_encoder_3d", name=f"train_cube_ae_{current_time}", mode="offline"
    )
    ...
    for step in tqdm(range(num_steps)):
        ...
        if not step % steps_til_plot:
            fig = plot_output_ground_truth(
                rgb[0], depth[0], gt_decoded_image[0], resolution=(64, 64, 3)
            )
