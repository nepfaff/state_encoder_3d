...
config = Namespace(
    log_path=f"outputs/planar_cube_info_nce_contrastive_{time.strftime('%Y-%b-%d-%H-%M-%S')}",
    checkpoint_path=f"outputs/planar_cube_info_nce_contrastive_{time.strftime('%Y-%b-%d-%H-%M-%S')}/checkpoints",
...
def main():
    ...
    wandb.init(
        ...
        name=f"train_planar_cube_info_nce_contrastive_{current_time}",
    ...
    for step in tqdm(range(config.num_steps)):
        ...
        neg_encoder_input = neg_image.view(
            config.batch_size, 10, config.img_res[0], config.img_res[1], 3
        ).permute(0, 1, 4, 2, 3)
        ...
        neg_latents = encoder(
            neg_encoder_input.reshape(-1, *neg_encoder_input.shape[-3:])
        ).reshape(
            config.batch_size, 10, config.latent_dim
        )  # Shape (B, 10, D)

        loss_ct: torch.Tensor = info_nce(anchor_latent, pos_latent, neg_latents)
        wandb.log(
            {
                "loss": loss_ct.item(),
            }
